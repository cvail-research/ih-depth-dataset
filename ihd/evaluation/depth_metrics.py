...
def valid_depth_mask(
    ...
    """Return pixels where prediction and target can be compared in meters."""
...
def median_scale_prediction(prediction_m: np.ndarray, target_m: np.ndarray, mask: np.ndarray) -> tuple[np.ndarray, float]:
    """Scale prediction by median(target) / median(prediction) on valid pixels."""
...
def affine_align_prediction(prediction_m: np.ndarray, target_m: np.ndarray, mask: np.ndarray) -> tuple[np.ndarray, float, float]:
    """Fit target ~= scale * prediction + shift on valid pixels."""
...
def depth_metrics_from_arrays(
    ...
    """Compute standard sparse metric-depth metrics.

    Inputs are metric depth/range maps in meters. Metrics are computed only on
    valid target pixels, which makes the function suitable for sparse projected
    LiDAR labels.
    """
...
def summarize_metric_rows(rows: list[Mapping[str, float]]) -> dict[str, float]:
    """Macro-average numeric metrics across scenes."""
    if not rows:
        return {}
