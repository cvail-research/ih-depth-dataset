...
<head>
  <meta charset="utf-8">
  <title>Annotation Workspace No-Cyl</title>
...
<body>
  ...
  <script type="module">
    ...
    function renderTargets() {
      ...
      picks.forEach((pick) => {
        ...
        div.onclick = async () => {
          ...
          statusFlash = 'Ctrl+click the matching LiDAR point to save';
    ...
    function updateStatus() {
      ...
      sceneSummary.textContent = `${scene.scene_label} | user targets: ${picks.length} | picked: ${picks.filter(p => p.status === 'picked').length}`;
      cloudStatus.textContent = pointcloud
        ...
        : 'Points rendered: ...';
      if (!active) {
        pickStatus.textContent = 'Click on the image to create a 2D target.';
      } else {
    ...
    function updatePointCloudOverlays() {
      ...
      pickedPointsObject = new THREE.Points(
        pickedGeometry,
        new THREE.PointsMaterial({ size: 9, sizeAttenuation: false, color: '#ff5d73', transparent: true, opacity: 0.95 }),
      );
      ...
      if (active) {
        ...
        const activeObject = new THREE.Points(
          activeGeometry,
          new THREE.PointsMaterial({ size: 12, sizeAttenuation: false, color: '#ffd166', transparent: true, opacity: 1.0 }),
        );
      ...
      candidatePointsObject = new THREE.Points(
        buildPointGeometry(candidatePoint ? [candidatePoint[0]] : [], candidatePoint ? [candidatePoint[1]] : [], candidatePoint ? [candidatePoint[2]] : []),
        new THREE.PointsMaterial({ size: 11, sizeAttenuation: false, color: '#7cfcff', transparent: true, opacity: 1.0 }),
      );
    ...
    async function handlePointPick(event) {
      ...
      candidatePoint = [hit.point[0], hit.point[1], hit.point[2]];
      statusFlash = activeIndex === null ? 'candidate selected; select a target first' : `candidate selected for #${activeIndex} with Ctrl+click (${hit.distancePx.toFixed(1)} px)`;
      updatePointCloudOverlays();
      updateStatus();
      if (activeIndex !== null) {
    ...
    async function loadScene() {
      ...
      activeIndex = scene.session ? scene.session.selected_target_index : null;
      imageElement.onload = () => {
        drawImageCanvas();
        renderTargets();
        updateStatus();
      ...
      if (timerIntervalId === null) {
        timerIntervalId = window.setInterval(async () => {
          ...
          clientTimerBaseSeconds = sessionState.elapsed_seconds || 0;
          if (sessionState.timing_running && clientTimerAnchorMs === null) clientTimerAnchorMs = Date.now();
          if (!sessionState.timing_running) clientTimerAnchorMs = null;
          updateStudyStatus();
        }, 1000);
      }
    ...
    canvas.addEventListener('click', async (event) => {
      ...
      if (event.altKey) {
        ...
        for (const pick of picks) {
          const dx = pick.image_uv[0] - image_uv[0];
          const dy = pick.image_uv[1] - image_uv[1];
        ...
        if (nearest !== null && bestDist <= 18) {
          ...
          statusFlash = `deleted image target #${nearest}`;
          await refreshPicks();
        }
        return;
      }
      ...
      statusFlash = 'target created; Ctrl+click the matching LiDAR point';
      await refreshPicks();
    });

    document.getElementById('savePickBtn').onclick = async () => { await saveCurrentCandidate(false); };
    document.getElementById('replacePickBtn').onclick = async () => { await saveCurrentCandidate(true); };
    ...
    document.getElementById('verdictGoodBtn').onclick = async () => {
      sessionState = await fetchJson('/api/verdict', { method: 'POST', headers: {'Content-Type': 'application/json'}, body: JSON.stringify({verdict: 'good'}) });
      updateStudyStatus();
    };
    document.getElementById('verdictCautionBtn').onclick = async () => {
      sessionState = await fetchJson('/api/verdict', { method: 'POST', headers: {'Content-Type': 'application/json'}, body: JSON.stringify({verdict: 'usable with caution'}) });
      updateStudyStatus();
    };
    document.getElementById('verdictBadBtn').onclick = async () => {
      sessionState = await fetchJson('/api/verdict', { method: 'POST', headers: {'Content-Type': 'application/json'}, body: JSON.stringify({verdict: 'bad'}) });
      updateStudyStatus();
    };
...
def create_app(workspace: NoCylSceneWorkspace) -> FastAPI:
    ...
    async def get_pointcloud() -> JSONResponse:
        try:
            ...
            return JSONResponse({"ready": False, "preprocessing": workspace.get_preprocess_status()}, status_code=409)
        return JSONResponse(payload)
    ...
    async def add_target(payload: TargetPayload) -> JSONResponse:
        return JSONResponse(workspace.add_target(payload.image_uv))

    @app.delete("/api/targets/{index}")
    async def delete_target(index: int) -> JSONResponse:
        try:
            return JSONResponse(workspace.delete_target(index))
        except KeyError as exc:
            raise HTTPException(status_code=404, detail=str(exc)) from exc

    @app.post("/api/session")
    async def update_session(payload: SessionPayload) -> JSONResponse:
        return JSONResponse(workspace.update_session(payload.selected_target_index))
    ...
    async def export_picks() -> JSONResponse:
        ...
        return JSONResponse({"export_csv": str(export_csv), "generated_corresp_txt": str(generated_corresp)})

    return app


def parse_args() -> argparse.Namespace:
    ap = argparse.ArgumentParser(description="Launch the single-scene no-cyl annotation workspace.")
...
def main() -> None:
    args = parse_args()
    workspace = NoCylSceneWorkspace(args.collection, args.path_name, args.step)
