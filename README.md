...
The repository includes a manifest builder:

```bash
uv run python ihd/datasets/build_frozen_manifest.py \
  --results-root analysis/lidar_labeling \
  --data-root /disk \
  --qc-reviews-csv configs/release/frozen_manifest_qc_reviews_template.csv \
  --overrides-csv configs/release/frozen_manifest_overrides_template.csv \
  --output-csv analysis/frozen_manifest_v0.csv \
  --output-markdown analysis/frozen_manifest_v0.md
```
